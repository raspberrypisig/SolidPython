...
class skin(OpenSCADObject, Bosl2Mixin):
...
class linear_sweep(OpenSCADObject, Bosl2Mixin):
...
class spiral_sweep(OpenSCADObject, Bosl2Mixin):
...
class path_sweep(OpenSCADObject, Bosl2Mixin):
...
class path_sweep2d(OpenSCADObject, Bosl2Mixin):
...
class sweep(OpenSCADObject, Bosl2Mixin):
...
class textured_linear_sweep(OpenSCADObject, Bosl2Mixin):
...
class textured_revolution(OpenSCADObject, Bosl2Mixin):
...
class textured_cylinder(OpenSCADObject, Bosl2Mixin):
...
class _taperfunc(OpenSCADObject, Bosl2Mixin):
    def __init__(self, x=None, **kwargs):
       super().__init__("_taperfunc", {"x" : x, **kwargs})

class _ss_polygon_r(OpenSCADObject, Bosl2Mixin):
...
class subdivide_and_slice(OpenSCADObject, Bosl2Mixin):
...
class slice_profiles(OpenSCADObject, Bosl2Mixin):
...
class rot_resample(OpenSCADObject, Bosl2Mixin):
...
class _dp_distance_array(OpenSCADObject, Bosl2Mixin):
...
class _dp_distance_row(OpenSCADObject, Bosl2Mixin):
...
class associate_vertices(OpenSCADObject, Bosl2Mixin):
...
class _get_texture(OpenSCADObject, Bosl2Mixin):
